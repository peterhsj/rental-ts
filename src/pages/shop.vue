<template>
  <div id="shopView" class="d-flex flex-column bg-grey-lighten-4 h-100">
    <!-- PC永遠顯示，Mobile選擇後才隱藏 -->
    <v-container v-show="!isMobile || !isEdit" class="pb-1 rental rental__wrapper">
      <v-card class="pa-2" elevation="4" variant="elevated">
        <v-img
          ...
          width="100%"
        >
          <h1 class="ml-6 mt-4 text-h5 font-weight-bold">商店消費停車折抵</h1>
        </v-img>
    ...
    <!-- 停車折抵 -->
    <v-container v-if="activeTab?.value === 'parkingDeduction' && memberInfo" id="parkingDeduction" class="rental rental__wrapper flex-grow-1 overflow-y-hidden">
      <ParkingDeduction
...
<script setup lang="ts">
  ...
  // 回到停車折抵主畫面
  function closeParkingDeduction (): void {
